/* Utilidade: Comportamento do Spider Jockey: dispara flechas prevendo fisica parabolica */
#include <math.h>
#include <stdlib.h>
...
/* Calcula a fisica parabolica da flecha para atingir o heroi. 
   O dano eh proporcional a velocidade relativa (inspirado no Minecraft). */

/* Garante que o valor fique dentro dos limites (minimo e maximo) */
static int sj_clamp(int v, int lo, int hi) {
    return v < lo ? lo : (v > hi ? hi : v);
}

/* Dispara uma flecha parabolica calculando o arco ate o alvo */
...
/* Move todas as flechas ativas aplicando gravidade e checando colisao com chao/paredes */
...
/* Inicializa as animacoes, ia e struct base do inimigo Spider Jockey */
...
/* IA do Spider Jockey: persegue, foge ou pula para atacar baseado na localizacao do alvo */
...
/* Desenha o inimigo e as flechas no ar. Usa tint vermelho se tomar dano recente */
...
/* Retorna a hitbox do esqueleto aranha para calculo de tiro do heroi batendo nele */
retangulo_t sj_hitbox(const spider_jockey_t *sj) {
    return (retangulo_t){ sj->px, sj->py, sj->frame->width, sj->frame->height };
}

/* Checa colisao de flechas com o heroi. O dano depende de onde bate e da inercia de movimento! */
int sj_flechas_colidir(spider_jockey_t *sj,
    ...
    for (int i = 0; i < SJ_FLECHAS_MAX; i++) {
        ...
        /* Se a flecha bater de frente com o jogador andando, da mais dano (calculo de velocidade relativa) */
        ...
        float v_rel = (alvo_vx - f->vx) * dir_x; /* Ignoramos o Y pra simplificar o dano no eixo horizontal */
...
/* Subtrai a vida do inimigo e aciona a animacao de piscar em vermelho */
...
/* Checa se a animacao final de morte acabou */

...
#define FASE3_TOTAL_INIMIGOS 30
    ...
    while (!fb_poll_quit()) {
        ...
        for (int i = 0; i < INIMIGOS_SIMULTANEOS; i++) {
            ...
            if (inimigo_esta_morto(&slot->inimigo)) {
                ...
                slot->ativo = 0;
                inimigos_mortos++;
                if (inimigos_spawnados < FASE3_TOTAL_INIMIGOS) {
        ...
        int inimigos_restantes = FASE3_TOTAL_INIMIGOS - inimigos_mortos;
        ...
        if (inimigos_mortos >= FASE3_TOTAL_INIMIGOS) {
            fase_terminando = 1;
        }
